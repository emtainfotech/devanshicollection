...
const ProductGrid = ({ title, subtitle, filter, limit = 4 }: ProductGridProps) => {
  ...
  let products = dbProducts?.length ? dbProducts : SAMPLE_PRODUCTS;
  if (!dbProducts?.length) {
    if (filter === 'trending') products = products.filter((p: any) => p.is_trending);
    if (filter === 'featured') products = products.filter((p: any) => p.is_featured);
