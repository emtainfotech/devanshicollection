-- Ensure dynamic settings table exists
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'chatbot_messages' AND policyname = 'Users can insert chatbot messages'
  ) THEN
    CREATE POLICY "Users can insert chatbot messages" ON public.chatbot_messages
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'chatbot_messages' AND policyname = 'Admins can view chatbot messages'
  ) THEN
    CREATE POLICY "Admins can view chatbot messages" ON public.chatbot_messages
